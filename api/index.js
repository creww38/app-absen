...
app.post('/api/siswa', authenticate, authorize(['admin']), async (req, res) => {
  try {
    ...
    await appendToSheet(process.env.SHEET_SISWA || 'siswa', [
      siswaData.nama, siswaData.nisn, siswaData.jenisKelamin || '',
      siswaData.tanggalLahir || '', siswaData.agama || '',
      siswaData.namaAyah || '', siswaData.namaIbu || '',
      siswaData.noHp || '', siswaData.kelas || '', siswaData.alamat || ''
    ]);
...
app.get('/api/siswa/kelas', async (req, res) => {
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][8]) kelasSet.add(data[i][8]);
    }
...
app.post('/api/absensi/scan', authenticate, async (req, res) => {
  try {
    ...
    if (!siswa) return res.json({ success: false, message: 'NISN tidak terdaftar' });
    ...
    if (!existingRecord) {
      ...
      return res.json({
        success: true, type: 'datang',
        message: isTerlambat ? `Absen Masuk (${keteranganWaktu})` : 'Absen Masuk Berhasil',
        jamDatang: nowTime, nama: siswa.nama, kelas: siswa.kelas
      });
    } else {
      ...
      return res.json({
        success: true, type: 'pulang', message: pesanPulang,
        jamPulang: nowTime, nama: siswa.nama, kelas: siswa.kelas
      });
    }
  } catch (error) {
    res.json({ success: false, message: error.message });
  ...
  try {
    ...
    for (let i = 1; i < absensiData.length; i++) {
      ...
      if (rowDateStr === todayStr && rowNisn === String(nisn).trim()) {
        return res.json({
          success: true, data: {
            tanggal: rowDateStr, jamDatang: absensiData[i][4],
            jamPulang: absensiData[i][5], keterangan: absensiData[i][6],
...
app.get('/api/monitoring/realtime', authenticate, async (req, res) => {
  try {
    ...
    for (let i = 1; i < siswaData.length; i++) {
      ...
      const info = absensiMap[nisn] || { jamDatang: '-', jamPulang: '-', keterangan: '-', status: 'Belum Absen' };
      result.push({
        nama: siswaData[i][0], nisn: nisn, kelas: kelas,
        jamDatang: info.jamDatang, jamPulang: info.jamPulang,
        keterangan: info.keterangan, status: info.status
      });
    }
    res.json({ success: true, data: result });
  } catch (error) {
  ...
  try {
    ...
    if (item) {
      ...
      while (currentDate <= endDate) {
        ...
        if (!exists) {
          await appendToSheet(process.env.SHEET_ABSENSI || 'absensi', [
            currentDate.toISOString(), nisn, nama, kelas, '-', '-', absensiKeterangan, absensiStatus
          ]);
        }
...
app.post('/api/export/excel', authenticate, authorize(['guru', 'admin']), async (req, res) => {
  try {
    ...
    if (type === 'absensi') {
      ...
      const monitoringData = await getSheetData(process.env.SHEET_SISWA || 'siswa');
      const todayStr = formatDateToYMD(new Date());
      const absensiData = await getSheetData(process.env.SHEET_ABSENSI || 'absensi');
      const absensiMap = {};
      for (let i = 1; i < absensiData.length; i++) {
        ...
        if (rowDate === todayStr) {
          absensiMap[nisn] = {
            jamDatang: absensiData[i][4] || '-', jamPulang: absensiData[i][5] || '-',
            keterangan: absensiData[i][6] || '-', status: absensiData[i][7] || 'Belum Absen'
          };
        }
      }
      let no = 1;
      for (let i = 1; i < monitoringData.length; i++) {
        const nisn = String(monitoringData[i][1] || '').replace(/^'/, '').trim();
        const info = absensiMap[nisn] || { jamDatang: '-', jamPulang: '-', keterangan: '-', status: 'Belum Absen' };
        data.push([no++, monitoringData[i][0] || '', nisn, monitoringData[i][8] || '-',
          info.jamDatang, info.jamPulang, info.keterangan, info.status]);
      }
    }
    ...
    const fileName = `${type}_${formatDateToYMD(new Date())}.xlsx`;
...
app.get('/', (req, res) => {
  res.json({ message: 'API Absensi Sekolah', version: '1.0.0', endpoints: '/api/auth/login, /api/siswa, /api/absensi/scan, /api/monitoring/realtime, /api/libur, /api/izin' });
});
